...
class FeatureExtractorPickle:
	...
	def _save_pickles(self, feature_path, x_train, y_train, f_train, x_test, y_test, f_test):
		import pandas as pd
		with open(os.path.join(feature_path, 'x_train.pkl'), 'wb') as f:
			pickle.dump(x_train, f)
		with open(os.path.join(feature_path, 'y_train.pkl'), 'wb') as f:
			multi_label = ['cur_label']
			for extra_label in self.dataset['extra_labels']:
				multi_label.append(extra_label)
			temp = pd.DataFrame(data=y_train, columns=multi_label)
			pickle.dump(temp, f)
		...
		with open(os.path.join(feature_path, 'y_test.pkl'), 'wb') as f:
			...
			for extra_label in self.dataset['extra_labels']:
				multi_label.append(extra_label)
			temp = pd.DataFrame(data=y_test, columns=multi_label)
			pickle.dump(temp, f)
	...
	def opensmile(self):
		...
		for i, row in self.dataset.train_data.iterrows():
			print('[Train] {}) Getting opensmile from {}...'.format(i, row['cur_name']), end='')
			wav_name = os.path.join(self.dataset['data_path'], row['cur_name'])
			try:
				x_train.append(vfe.process(wav_name))
		...
		for i, row in self.dataset.test_data.iterrows():
			print('[Test] {}) Getting opensmile from {}...'.format(i, row['cur_name']), end='')
			wav_name = os.path.join(self.dataset['data_path'], row['cur_name'])
			try:
				x_test.append(vfe.process(wav_name))
		...
		# 	pickle.dump([x_train,y_train,f_train,x_test,y_test,f_test], f, protocol=2)
